  ...
  return (
    <div className='rounded bg-[#1a1a1a] border border-neutral-800/60 p-4 space-y-3'>
      <div>
        <p
          ref={questionRef}
          className={`text-sm font-medium text-white leading-relaxed ${questionExpanded ? "" : "line-clamp-3"}`}
        >
          {item.question}
        </p>
        {questionOverflows && (
      ...
      <div>
        <p
          ref={adviceRef}
          className={`text-sm text-neutral-400 leading-relaxed ${adviceExpanded ? "" : "line-clamp-3"}`}
        >
          {item.advice}
        </p>
        {adviceOverflows && (
...
function GapCard({ item }: { item: GapSuggestion }) {
  ...
  useEffect(() => {
    if (recRef.current)
      setOverflows(recRef.current.scrollHeight > recRef.current.clientHeight);
  }, []);
  ...
  return (
    <div className='rounded bg-[#1a1a1a] border border-neutral-800/60 p-4 space-y-3'>
      ...
      <div>
        ...
        <p
          ref={recRef}
          className={`text-sm text-neutral-400 leading-relaxed ${expanded ? "" : "line-clamp-4"}`}
        >
          {item.recommendation}
        </p>
        {overflows && (
