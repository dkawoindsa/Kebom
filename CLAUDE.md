...
- Tailwind CSS v4
- Anthropic Claude API (claude-sonnet-4-6 파싱, claude-opus-4-7 분석)
- pdf-parse (서버 전용 API 라우트에서만 사용)
- Jest + React Testing Library

## 아키텍처 규칙
- CRITICAL: 모든 AI API 호출과 PDF 처리는 반드시 app/api/ 라우트 핸들러에서만 수행한다. 클라이언트 컴포넌트에서 Anthropic SDK를 직접 임포트하거나 호출하지 마라.
- CRITICAL: ANTHROPIC_API_KEY는 process.env에서만 읽는다. 클라이언트 번들에 노출되지 않도록 NEXT_PUBLIC_ 접두사를 붙이지 마라.
...
- `process.env.ANTHROPIC_API_KEY`는 `app/api/` 라우트와 `src/lib/ai/` 에서만 참조
- 키 미설정 시 API 라우트에서 즉시 500 반환 (키 없이 Anthropic SDK 호출 시도 금지)

## 프로젝트 부트스트랩
소스 코드가 없는 상태에서 처음 시작할 때 순서:
1. `package.json`, `tsconfig.json`, `next.config.ts`, `jest.config.ts` 생성
2. `npm install`
3. `.env.local` 생성 후 ANTHROPIC_API_KEY 입력
4. `npm run dev`로 확인
...
- Anthropic SDK 모킹: `jest.mock('@anthropic-ai/sdk')`
