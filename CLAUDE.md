...
- CRITICAL: 모든 AI API 호출과 PDF 처리는 반드시 src/app/api/ 라우트 핸들러에서만 수행한다. 클라이언트 컴포넌트에서 Google Generative AI SDK를 직접 임포트하거나 호출하지 마라.
- CRITICAL: GOOGLE*AI_API_KEY는 process.env에서만 읽는다. 클라이언트 번들에 노출되지 않도록 NEXT_PUBLIC* 접두사를 붙이지 마라.
...
- API 라우트는 src/app/api/ 하위에만 둔다. 파일 업로드 파싱은 API 라우트에서 처리하고 완료 즉시 메모리에서 해제한다 (임시 파일 디스크 저장 금지).
- CRITICAL: pdf-parse는 Node.js API를 사용하므로 Edge Runtime에서 임포트 금지. parse-resume 라우트만 `runtime = 'nodejs'`, 나머지(analyze, parse-jd)는 `runtime = 'edge'`를 유지한다.
...
# Upstash Redis — 레이트 리미팅용 (선택, 미설정 시 rate limiting 비활성화)
...
`src/middleware.ts`가 모든 `/api/*` 경로에 IP 기반 슬라이딩 윈도우 10 req/min 제한을 적용한다.
- Upstash 환경 변수 미설정 시 로컬에서는 자동 비활성화 (경고 로그만 출력)
- 프로덕션에서는 반드시 Upstash 설정 필요
- analyze 라우트: `maxDuration = 30`초, parse-resume: `maxDuration = 10`초 (Vercel 제한)
